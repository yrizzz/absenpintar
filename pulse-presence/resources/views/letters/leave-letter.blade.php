...
<head>
    <meta charset="UTF-8">
    <title>Surat Permohonan Cuti - {{ $leave->user->name ?? 'Karyawan' }}</title>
    <style>
...
<body>
    @php
        ...
        $leaveLabel = $leaveTypes[$leave->leave_type] ?? ucfirst($leave->leave_type);

        $statusLabels = [
            'pending' => 'Menunggu Persetujuan',
            'hr_approved' => 'Disetujui',
            'manager_approved' => 'Disetujui Manajer',
            'rejected' => 'Ditolak',
        ];
        $statusLabel = $statusLabels[$leave->status] ?? ucfirst($leave->status);
        $statusClass = in_array($leave->status, ['hr_approved', 'manager_approved']) ? 'badge-approved' : ($leave->status === 'rejected' ? 'badge-rejected' : 'badge-pending');

        $nomorSurat = 'CT/' . str_pad($leave->id, 4, '0', STR_PAD_LEFT) . '/HRD/' . \Carbon\Carbon::now()->format('m/Y');
    @endphp
    ...
    <div class="isi">
        ...
        <table class="data-table">
            ...
            <tr>
                ...
                <td>:</td>
                <td>{{ $leave->user->branch->name ?? 'Kantor Pusat' }}</td>
            </tr>
            <tr>
                <td>Jabatan / Peran</td>
                <td>:</td>
                <td>{{ ucfirst($leave->user->roles->first()?->name ?? 'Karyawan') }}</td>
            </tr>
        </table>
        ...
        <table class="data-table">
            <tr>
                <td>Jenis Cuti</td>
                <td>:</td>
                <td><strong>{{ $leaveLabel }}</strong></td>
            </tr>
            <tr>
                <td>Tanggal Mulai</td>
                <td>:</td>
                <td>{{ \Carbon\Carbon::parse($leave->start_date)->translatedFormat('l, d F Y') }}</td>
            </tr>
            <tr>
                <td>Tanggal Selesai</td>
                <td>:</td>
                <td>{{ \Carbon\Carbon::parse($leave->end_date)->translatedFormat('l, d F Y') }}</td>
            </tr>
            <tr>
                <td>Durasi</td>
                <td>:</td>
                <td><strong>{{ $leave->total_days }} Hari Kerja</strong></td>
            </tr>
            <tr>
        ...
        @if($leave->hr_notes || $leave->manager_notes)
        <div class="catatan">
            <strong>Catatan Pejabat Berwenang:</strong><br>
            @if($leave->manager_notes)
                <em>Manajer: "{{ $leave->manager_notes }}"</em><br>
            @endif
            @if($leave->hr_notes)
                <em>HRD: "{{ $leave->hr_notes }}"</em>
            @endif
        </div>
        @endif
    </div>
    ...
    <div class="ttd-container">
        <div class="ttd-box">
            ...
            <div class="garis"></div>
            <p class="nama">{{ $leave->user->name ?? 'N/A' }}</p>
            <p class="jabatan">ID: {{ $leave->user->employee_id ?? 'N/A' }}</p>
        </div>
        <div class="ttd-box">
            <p class="label">Menyetujui,</p>
            <div class="garis"></div>
            <p class="nama">{{ $leave->hr->name ?? $leave->manager->name ?? '___________________' }}</p>
